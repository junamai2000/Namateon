...
#define HTTP_RESPONSE_200_DUMMY ""                 \
	...
	"Server: " MODULE_IDENTITY "/" MODULE_REV CRLF     \
...
extern namateon_server_config* server_config;
...
http_server_send (struct socket *sock, const char *buf, size_t size, int more) {
	...
	while (done < size) {
		...
		if (length < 0) {
			printk(KERN_ERR MODULE_IDENTITY ": write error: %d done=%d\n", length, done);
			break;
		}
		done += length;
	}
...
open_client_socket (const char *addr, ushort port, struct socket **res) {
	...
	if (err < 0) {
		printk(KERN_ERR MODULE_IDENTITY ": kernel_connect() failure, err=%d\n", err);
		sock_release(sock);
		return err;
	}
	*res = sock;
...
redirect_response (struct http_request *request, int keep_alive) {
	...
	while (1) {
		...
		if (len <= 0) {
			if (len) {
				printk(KERN_ERR MODULE_IDENTITY ": recv error: %d\n", len);
			}
			break;
		}
...
redirect_get_request (struct http_request *request, int keep_alive) {
	...
	if (!request->proxy_socket)	{
        list_for_each(p, &server_config->proxy_list.list) {
            tmp = list_entry(p, reverse_proxy, list);
            printk(KERN_ERR MODULE_IDENTITY ": %s -> %s:%d\n",request->request_url, tmp->proxy_server, tmp->proxy_port);
            if(!strncmp(request->request_url, tmp->path, strlen(tmp->path)))
            {
                printk(KERN_ERR MODULE_IDENTITY ": match proxy: %s -> %s:%d\n",request->request_url, tmp->proxy_server, tmp->proxy_port);
                err = open_client_socket(tmp->proxy_server, tmp->proxy_port, &request->proxy_socket);
                if (err < 0) {
                    printk(KERN_ERR MODULE_IDENTITY ": can't open client socket!\n");
                    return 500;
                }
            }
        }
...
http_server_response (struct http_request *request, int keep_alive) {
	...
	printk(KERN_INFO MODULE_IDENTITY ": request_url = %s\n", request->request_url);
...
http_server_worker (void *arg) {
	...
	if (!request) {
		printk(KERN_ERR MODULE_IDENTITY ": can't allocate memory!\n");
		return -1;
	}
	request->send_bufsize = 0;
...
http_server_daemon (void *arg) {
	...
	while (!kthread_should_stop()) {
		...
		if (err < 0) {
			...
			printk(KERN_ERR MODULE_IDENTITY ": kernel_accept() error: %d\n", err);
			continue;
		}
		kernel_setsockopt(socket, SOL_TCP, TCP_NODELAY, (void*)&yes, sizeof(yes));
		worker = kthread_run(http_server_worker, socket, MODULE_IDENTITY);
		if (IS_ERR(worker)) {
			printk(KERN_ERR MODULE_IDENTITY ": can't create more worker process\n");
			continue;
		}
	}
	return 0;
