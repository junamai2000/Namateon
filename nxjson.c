...
#define NX_JSON_CALLOC() kmalloc( sizeof(nx_json), GFP_KERNEL)
#define NX_JSON_FREE(json) kfree((void*)(json))
#endif

// redefine NX_JSON_REPORT_ERROR to use custom error reporting
#ifndef NX_JSON_REPORT_ERROR
#define NX_JSON_REPORT_ERROR(msg, p) printk(KERN_ERR "NXJSON PARSE ERROR (%d): " msg " at %s\n", __LINE__, p)
#endif
...
static nx_json* create_json(nx_json_type type, const char* key, nx_json* parent) {
  ...
  if(!js)
  {
      printk(KERN_INFO MODULE_IDENTITY "Fail to allocate memory : type=%d, char=%s", type, key);
  }
      ...
      case '-': case '0': case '1': case '2': case '3': case '4': case '5': case '6': case '7': case '8': case '9':
        {
          ...
          js->int_value=simple_strtol(p, &pe, 0);
          if (pe==p) {
